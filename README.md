...
Though this is a fully functional neural network toolkit which can be used to structure and train neural networks with different architectures and with different optimizers and regularizers.

## Project structure

```
digit-classification/
├── main.py               # call necessary functions
├── requirements.txt
├── data/                  # mnist_train_small.csv / mnist_test.csv here
└── src/
    ├── layers.py          # Linear, BatchNorm1d, activation functions, Sequential
    ├── model.py            # define model architecture here
    ├── data.py             # train-val-test split and basic preprocessing
    ├── engine.py           # actual training loop + accuracy calculations
    ├── optim.py           # contains all pytorch inspired optimizer classes
...
```bash
git clone https://github.com/RatSak5/Digit-Classifier-MLP
pip install -r requirements.txt
```

Download `mnist_train_small.csv` and `mnist_test.csv` from here and place in `data/`.

## Usage

call the python file as
```bash
python main.py
```

Optional flags to play with :
```bash
...
Although, I got a lower training loss on Mini-Batch Gradient Descent optimized model. The following are the results:
...
- Handwritten backward functionality.
- Adding even more optimizers and regularizers.
- Maybe this project will turn into something totally different. Or it might not.
